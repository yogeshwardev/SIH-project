...
export default function AuthModal({
  ...
  const [sellerRegion, setSellerRegion] = useState('Varanasi, Uttar Pradesh');

  // Admin Form
  const [adminId, setAdminId] = useState('GOV-OFFICER-8841');
  const [adminKey, setAdminKey] = useState('GovtPass@2026');
  const [officerName, setOfficerName] = useState('Dr. Anand Varma (Director of GI Standards)');
    ...
    try {
      if (isRegisterMode) {
        const res = await api.registerBuyer({
          name: buyerName || 'CraftLink Buyer',
          email: buyerEmail || 'buyer@craftlink.in',
          phone: buyerPhone || '+91 98765 43210',
          password: buyerPassword || 'password',
          pincode: '110001',
          city: 'New Delhi'
        });
        setSuccessMsg('Account created successfully! Welcome to CraftLink.');
        setTimeout(() => {
        ...
        const res = await api.login({
          email_or_phone: buyerEmail || 'buyer@craftlink.in',
          password: buyerPassword || 'password',
          role: 'buyer'
        });
  ...
  const handleSellerQuickRegister = async (e) => {
    ...
    try {
      const res = await api.registerSeller({
        owner_name: sellerOwnerName || 'Master Weaver Ramesh',
        store_name: sellerStoreName || 'Varanasi Heritage Handlooms',
        email: sellerEmail || 'seller@varanasiguild.in',
        phone: sellerPhone || '+91 98765 11223',
        craft_category: sellerCategory,
        region: sellerRegion,
        artisan_card_number: 'IND-PEHCHAN-2026-9821',
        bank_account: '98765432101234',
        ifsc_code: 'SBIN0001234',
        pincode: '221001'
      });
      setSuccessMsg('Seller Store Created & Verified! Redirecting to Seller Central...');
      setTimeout(() => {
        onLoginSuccess?.(res.user, 'seller');
        onClose();
      }, 1200);
    } catch (err) {
      setError(err.message || 'Seller onboarding failed.');
    } finally {
      setLoading(false);
    }
  };
            ...
            <div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                <span className="text-[10px] font-bold uppercase text-gray-400 tracking-wider block mb-2">
                  One-Click Sample Profiles:
                </span>
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => quickFillBuyer('priya.sharma@example.com', 'Priya Sharma')}
                    className="flex-1 py-1.5 px-2 bg-gray-50 border border-gray-200 hover:border-amber-400 rounded text-[11px] font-medium text-gray-700 text-left truncate"
                  >
                    👤 Priya Sharma (Delhi)
                  </button>
                  <button
                    type="button"
                    onClick={() => quickFillBuyer('rahul.verma@example.com', 'Rahul Verma')}
                    className="flex-1 py-1.5 px-2 bg-gray-50 border border-gray-200 hover:border-amber-400 rounded text-[11px] font-medium text-gray-700 text-left truncate"
                  >
          ...
          {activeTab === 'seller' && (
            <div>
              <div className="mb-4">
                ...
                <h3 className="text-base font-bold text-gray-900" style={{ fontFamily: "'Outfit', sans-serif" }}>
                  Register / Sign In as Master Artisan
                </h3>
                <p className="text-xs text-gray-500">
                  Launch your online artisan store with instant AI voice cataloging.
                </p>
              </div>
              ...
              <form onSubmit={handleSellerQuickRegister} className="space-y-2.5">
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] font-bold text-gray-600 uppercase mb-1">Master Artisan Name</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-600 uppercase mb-1">Artisan Cluster / Region</label>
                  <input
                    type="text"
                ...
                >
                  {loading ? 'Creating Store...' : 'Launch Store & Open Seller Central'}
                  <ArrowRight className="w-3.5 h-3.5" />
                </button>
              </form>

              {/* Quick Select Clusters */}
              <div className="mt-3 pt-3 border-t border-gray-100">
                <span className="text-[10px] font-bold uppercase text-gray-400 tracking-wider block mb-1.5">
                  Select Existing Verified Guilds:
                </span>
                <div className="grid grid-cols-2 gap-1.5">
                  <button
                    type="button"
                    onClick={() => quickFillSeller('Ramesh Weaver', 'Varanasi Master Weavers Hub', 'Handloom & Textiles', 'Varanasi, UP')}
                    className="py-1 px-2 bg-gray-50 hover:bg-orange-50 border border-gray-200 rounded text-[11px] font-medium text-gray-800 text-left truncate"
                  >
                    🧣 Varanasi Weavers
                  </button>
                  <button
                    type="button"
                    onClick={() => quickFillSeller('Govind Potter', 'Jaipur Blue Pottery Guild', 'Pottery & Ceramics', 'Jaipur, Rajasthan')}
                    className="py-1 px-2 bg-gray-50 hover:bg-orange-50 border border-gray-200 rounded text-[11px] font-medium text-gray-800 text-left truncate"
                  >
                    🏺 Jaipur Pottery
                  </button>
                  <button
                    type="button"
                    onClick={() => quickFillSeller('Somu Murmu', 'Bastar Dhokra Tribal Collective', 'Metal Craft & Bell Metal', 'Bastar, Chhattisgarh')}
                    className="py-1 px-2 bg-gray-50 hover:bg-orange-50 border border-gray-200 rounded text-[11px] font-medium text-gray-800 text-left truncate"
                  >
                    🔱 Bastar Tribal
                  </button>
                  <button
                    type="button"
                    onClick={() => quickFillSeller('Nagesh Rao', 'Channapatna Craft Society', 'Woodcraft & Carving', 'Ramanagara, Karnataka')}
                    className="py-1 px-2 bg-gray-50 hover:bg-orange-50 border border-gray-200 rounded text-[11px] font-medium text-gray-800 text-left truncate"
                  >
                    🪵 Channapatna Toys
                  </button>
              ...
              {onNavigateToSellerOnboarding && (
                <div className="mt-3 text-center">
                  ...
                  >
                    Want full GST/KYC onboarding? Open Full Seller Registration Form →
                  </button>
                </div>
              )}
            </div>
